...
const WasteAnalysis = () => {
  ...
  // Waste reasons are damage, expired, theft
  const wasteAdjustments = adjustments.filter(adj => 
    ['damage', 'expired', 'theft'].includes(adj.reason)
  );

  const totalWasteCount = wasteAdjustments.length;
  const totalWasteQty = wasteAdjustments.reduce((sum, adj) => sum + Math.abs(Number(adj.qty_change)), 0);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text-primary)' }}>Waste Analysis</h1>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }}>
        <StatCard 
          label="Total Waste Transactions"
          value={totalWasteCount}
          icon={Trash2}
        />
        <StatCard 
          label="Total Units Wasted"
          value={totalWasteQty}
          icon={TrendingDown}
        />
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
          <thead style={{ background: 'var(--bg-input)' }}>
            <tr>
              ...
              <th style={{ textAlign: 'right', padding: '14px 18px', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', borderBottom: '1px solid var(--border)' }}>Quantity</th>
          ...
          <tbody>
            ...
            <tr key={adj.id} style={{ transition: 'background 0.15s ease' }} onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-input)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
              ...
              <td style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-light)', textAlign: 'right', fontSize: '14px', color: '#dc2626', fontWeight:600 }}>
                {Math.abs(Number(adj.qty_change))}
              </td>
            ...
            {wasteAdjustments.length === 0 && (
              <tr>
                <td colSpan={5} style={{ padding: '48px', textAlign: 'center', color: 'var(--text-muted)' }}>
                  No waste recorded yet. Keep it up!
                </td>
              </tr>
            )}
          </tbody>
