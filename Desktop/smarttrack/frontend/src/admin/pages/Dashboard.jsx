...
import { getAlerts, getProducts, getSalesHistory, getStock } from '../api/adminApi';
import { useAdminAuth } from '../context/AdminAuthContext';
import StatCard from '../../components/ui/StatCard';
import AlertPanel from '../../components/ui/AlertPanel';
import { AlertTriangle, BarChart3, DollarSign, Package, ShoppingBag, Tag, TrendingDown } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [alertsRes, salesRes, productsRes, stockRes] = await Promise.all([getAlerts(), getSalesHistory(), getProducts(), getStock()]).catch(() => [
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>Dashboard</h1>
        <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '4px' }}>
          Welcome back, {adminUser?.name}. Here's what's happening in your store.
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
        <StatCard icon={Package} label="Total Products" value={stats.products} trend={`${stats.batches} batches`} trendUp={true} color="#2563eb" bgColor="rgba(37, 99, 235, 0.08)" />
        <StatCard icon={ShoppingBag} label="Total Items Sold" value={stats.totalSales} trend={`${recentSales.length} recent`} trendUp={true} color="#10b981" bgColor="rgba(16, 185, 129, 0.08)" />
        <StatCard
          icon={TrendingDown}
          label="Low Stock"
        ...
        />
        <StatCard icon={DollarSign} label="Total Revenue" value={`₱${stats.revenue.toFixed(2)}`} trend={`${allSales.length} transactions`} trendUp={true} color="#8b5cf6" bgColor="rgba(139, 92, 246, 0.08)" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}>
        <AlertPanel title="Low Stock" items={alerts.low_stock} icon={AlertTriangle} emptyText="All items well stocked" color="#f59e0b" />
        <AlertPanel title="Expired" items={alerts.expired} icon={AlertTriangle} emptyText="No expired items" color="#ef4444" />
