...
const StatCard = ({ icon: Icon, label, value, trend, trendUp, color, bgColor }) => (
  ...
  >
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
      <div>
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: 500, marginBottom: '8px' }}>{label}</p>
        <p
          style={{
            fontSize: '28px',
            fontWeight: 700,
        ...
        {trend && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '10px' }}>
            {trendUp ? <ArrowUpRight size={14} color="var(--success)" /> : <ArrowDownRight size={14} color="var(--warning)" />}
            <span
              style={{
                fontSize: '12px',
                fontWeight: 500,
