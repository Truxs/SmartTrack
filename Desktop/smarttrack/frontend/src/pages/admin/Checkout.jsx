...
import { getProductByBarcode, checkout as checkoutApi, getProducts } from '../../api/api';
import { ScanBarcode, ShoppingCart, Trash2, Check, X, Search, Plus, Minus, AlertCircle } from 'lucide-react';
...
const Checkout = () => {
    ...
    const processCheckout = async () => {
        ...
        if (errors.length > 0) {
            setMessage({
                type: 'error',
                text: errors.map(e => `${e.product}: ${e.error}`).join('; ')
            });
        }
        ...
        if (results.length > 0) {
            setLastReceipt({
                items: results,
                total: results.reduce((s, r) => s + parseFloat(r.total_price), 0),
                date: new Date()
            });
            // Remove successfully sold items from cart
            const soldBarcodes = results.map(r => r.product.barcode);
            setCart(prev => prev.filter(item => !soldBarcodes.includes(item.barcode)));

            if (errors.length === 0) {
                setMessage({ type: 'success', text: `Sale completed! Total: ₱${results.reduce((s, r) => s + parseFloat(r.total_price), 0).toFixed(2)}` });
            }
        }
        ...
        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div>
                ...
                <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '4px' }}>
                    Scan barcode or search products to process a sale
                </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '24px', alignItems: 'start' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <input
                                ...
                                style={{
                                    ...
                                    borderRadius: 'var(--radius-md)', padding: '12px 16px', fontSize: '15px',
                                    color: 'var(--text-primary)', outline: 'none', fontFamily: 'monospace',
                                    letterSpacing: '0.05em'
                                }}
                            />
                            <button onClick={() => lookupProduct(barcode)} style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ maxHeight: '300px', overflowY: 'auto' }}>
                                {filteredProducts.map(p => (
                                    <div key={p.id} onClick={() => { addToCart(p); setShowProductPicker(false); setPickerSearch(''); }}
                                        className="picker-item" style={{
                                        ...
                                        <div>
                                            ...
                                            <p style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{p.barcode} • Stock: {p.total_stock || 0}</p>
                                        </div>
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                                ...
                                <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                                    {lastReceipt.date.toLocaleString()}
                                </span>
                            </div>
                            {lastReceipt.items.map((item, i) => (
                                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--border-light)', fontSize: '13px' }}>
                                    <span style={{ color: 'var(--text-secondary)' }}>{item.product.name} × {item.quantity_sold}</span>
                ...
                <div style={{
                    ...
                    boxShadow: 'var(--shadow-sm)', position: 'sticky', top: '92px'
                }}>
                    {/* Cart Header */}
                            ...
                            <div key={item.id} style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0', borderRadius: '8px', border: '1px solid var(--border)', overflow: 'hidden' }}>
                                        <button onClick={() => updateQty(item.id, -1)} style={qtyBtn}><Minus size={14} /></button>
                                        ...
                                        <button onClick={() => updateQty(item.id, 1)} style={qtyBtn}><Plus size={14} /></button>
                                    </div>
                    ...
                    <div style={{ padding: '20px', borderTop: '1px solid var(--border-light)', background: 'var(--bg-input)' }}>
                        ...
                        <button onClick={processCheckout} disabled={cart.length === 0 || processing} style={{
                            ...
                            fontSize: '15px', fontWeight: 700, cursor: cart.length === 0 ? 'not-allowed' : 'pointer',
                            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
                            boxShadow: cart.length > 0 ? '0 2px 8px rgba(37,99,235,0.3)' : 'none'
                        }}>
                            {processing ? 'Processing...' : <><Check size={18} /> Process Sale</>}
                        </button>
                    </div>
                </div>
            </div>
